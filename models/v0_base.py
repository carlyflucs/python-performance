...
class Model:
    ...
    def run(self, inputs, seed, iterations):
        ...
        self.item_minutes_rate = scipy.stats.norm.rvs(loc=avg_rate, scale=avg_rate/10, random_state=self.random_state, size=1)
    ...
    def process_item(self, item):
        ...
        find_time = scipy.stats.norm.rvs(loc=self.item_minutes_rate, scale=self.item_minutes_rate/2, random_state=self.random_state)
        # Minimum item time of 10% item rate
        find_time = max(find_time, self.item_minutes_rate*0.1) 

        item_bagged = False
        # Did I find the item?
        found = scipy.stats.binom.rvs(n=1, p=item["find_prob"], size=1, random_state=self.random_state)
        if found:
            # I've found the item, now can I reach it?
            reached = scipy.stats.binom.rvs(n=1, p=item["reach_prob"], size=1, random_state=self.random_state)
            if reached:
                item_bagged = True

        # Check for a last second granny block
        blocked = scipy.stats.binom.rvs(n=1, p=self.core_inputs["blocked_by_granny_prob"], size=1, random_state=self.random_state)
        if blocked:
            # Calculate the time I was waiting 
            block_time = scipy.stats.norm.rvs(loc=self.item_minutes_rate, scale=self.item_minutes_rate/2, random_state=self.random_state)
